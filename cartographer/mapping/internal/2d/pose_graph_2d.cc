...
NodeId PoseGraph2D::AddNode(
    ...
    const int trajectory_id,
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) {
  //  将节点在local坐标系下的坐标转成global坐标系下的坐标; 生成一个新的Pose，这个Pose是由trajectory相对于世界坐标系的Pose乘以节点数据中包含的该节点相对于该trajectory的LocalTrajectoryBuilder Pose
...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
  ...
  const Submap2D* submap;
  {
    ...
    } else if (global_localization_samplers_[node_id.trajectory_id]->Pulse()) {  // 如果节点与子图不属于同一条轨迹 并且 间隔了一段时间, 同时采样器为true 才进行 全局搜索窗口 的约束计算(对整体子图进行回环检测)
  ...
  } else if (maybe_add_global_constraint) {// 定位时才有可能执行这块
...
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
  ...
  if (begin_it == end_it) {
    ...
    } else {	// note: 没设置初始位姿就将返回(0,0,0)的平移和旋转
  ...
  // delete their corresponding constraint submap matchers to save memory.
  {
    ...
    for (const Constraint& constraint : parent_->data_.constraints) {
      if (constraint.tag == Constraint::Tag::INTRA_SUBMAP) {
        continue;
      } else if (other_submap_ids_losing_constraints.count(// 只要other_submap_ids_losing_constraints内submap_id还存在其他的子图间约束就把这个子图id从other_submap_ids_losing_constraints中删除, 可以留着
                     constraint.submap_id)) {
