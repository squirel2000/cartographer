...
LocalTrajectoryBuilder2D::AddRangeData(
  ...
  for (size_t i = 0; i < synchronized_data.ranges.size(); ++i) {
    ...
    // 将点云的origins坐标转到 local slam 坐标系下; Convert the synchronized range data into the pose w.r.t the robot
    const Eigen::Vector3f origin_in_local =
        range_data_poses[i] *
        synchronized_data.origins.at(synchronized_data.ranges[i].origin_index);

    // Step: 3 运动畸变的去除, 将相对于tracking_frame的hit坐标 转成 local坐标系下的坐标; Calculate the difference between hit_in_local and origin_in_local (origin of the local trajectory?)
  ...
  // Insert the AccumulatedRangeData, in this case num_accumulated_range_data == 1 几帧有效的点云数据进行一次扫描匹配
...
LocalTrajectoryBuilder2D::AddAccumulatedRangeData(
  ...
  // 计算耗时; Estimate the latencyMetric and cpu_time
...
LocalTrajectoryBuilder2D::InsertIntoSubmap(
  ...
  // InsertionResult including TrajectoryNode and submaps; 生成InsertionResult格式的数据进行返回
